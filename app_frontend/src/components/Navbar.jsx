        ...
        <div className='relative z-20 w-full sm:w-[97%] md:w-[80%] lg:w-[70%] mx-auto'>
            <div className='bg-[#0A0A0A] flex justify-between px-4 py-4 items-center' style={{ borderBottom: '1px solid transparent', borderImage: 'linear-gradient(to right, transparent, rgba(217,169,25,0.3) 20%, rgba(245,197,23,0.6) 50%, rgba(217,169,25,0.3) 80%, transparent) 1' }}>
                ...
                <div className='hidden md:flex gap-2 items-center'>
                    ...
                    <Link to='/signupEmail' className='group px-5 py-2 bg-[#1f1d10] flex gap-1 items-center justify-center hover:bg-[#2b280f] rounded-xl text-[#D9A919] duration-300 border border-[#65531e]'>Signup <ArrowRight className='group-hover:translate-x-0.5 transition-transform' size={17} /></Link>
                </div>
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-300 bg-[#0A0A0A] border-b border-[#211e12] ${open ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 border-none'}`}>
                ...
                <Link to='/signupEmail' onClick={() => setOpen(false)} className='group mt-1 px-4 py-2 bg-[#1f1d10] flex gap-1 items-center justify-center hover:bg-[#2b280f] rounded-xl text-[#D9A919] duration-300 border border-[#65531e] w-full'>Signup <ArrowRight className='group-hover:translate-x-0.5 transition-transform' size={17} /></Link>
                </div>
            </div>
        </div>
    );
